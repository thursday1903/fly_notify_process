...
public class EmailProcess {
	 ...
	 * @throws Exception
	 */
	public int sendEmail(NotifyMessageTransfer message) throws Exception {
		try {
			// String spName = "";
			String to = message.getReceive_email_expect();
			String from = MainConfig.mailFrom;
			// String Path = "";
			String msgText1 = message.getContent();
			...
			InternetAddress[] addressTo = convertListEmail(to);
			InternetAddress[] addressCc = convertListEmail(message.getReceive_email_expect_cc());
			InternetAddress[] addressBCc = convertListEmail(message.getReceive_email_expect_bcc());
			
			msg.setRecipients(javax.mail.Message.RecipientType.TO, addressTo);
			msg.setRecipients(javax.mail.Message.RecipientType.CC, addressCc);
			msg.setRecipients(javax.mail.Message.RecipientType.CC, addressBCc);
			...
			try {
				Transport.send(msg);
				log.info("gui email thanh cong toi danh sach: " + message.getReceive_email_expect());
